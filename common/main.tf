    ...
    volumes_map = {
      ...
      beluga = {
        nfs = {
          ...
          scratch  = { size = try(local.custom.scratch_size, local.default_pod.scratch_size), type = "volumes-ec", quota = try(local.custom.user_quotas.scratch, local.default_pod.user_quotas.scratch), managed = true  }
        }
      }
      juno = {
        nfs = {
