    ...
    volumes_map = {
      ...
      beluga = {
        nfs = {
          ...
          perm_project = { size = try(local.custom.project_size, local.default_pod.project_size), type = "volumes-ec", quota = try(local.custom.user_quotas.project, local.default_pod.user_quotas.project), managed = false }
